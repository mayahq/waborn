 ...
 */
  public async setGroupIcon(groupId: GroupChatId, b64: Base64) {
    ...
    if(!mimeInfo || mimeInfo.includes("image")){
      ...
      const scaledImageBuffer = await lipo(buff,{ failOnError: false })
      .resize({ height: 300 })
      .toBuffer();
      const jpeg = lipo(scaledImageBuffer,{ failOnError: false }).jpeg();
  ...
  */
  ...
  private async prepareWebp(b64: DataURL) {
    ...
    if(!mimeInfo || mimeInfo.includes("image")){
      ...
      .toBuffer();
      const webp = lipo(scaledImageBuffer,{ failOnError: false }).webp();
